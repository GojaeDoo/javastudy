...
/*
   ...
   File 클래스 : 특정위치에 존쟇는 파일이나 디렉토리(폴더)를 처리하는 클래스
   - 주요 생성자 : File (String 경로) , File (String 상위경로 , String 하위 경로)
                                    File (File 상위경로 , String 하위 경로)
                                    
   - 주요 메서드 : createNewFile() : createNewFile() : 파일생성
                                  mkdir() , mkdirs() : 디펙토리 생성
                                  delete() : 삭제
                                  isFile() : 파일이면 true
                                  isDirectory() : 디펙토리 true
                                  exists() : 있으면 true
                                  getName(): 이름 반환
                                  length() : 파일의 크기를 byte로 변환 (디펙토리는 크기가 없음
...
public class Ex06 {
	public static void main(String[] args) {
		// 본인 환경에 맞게 경로 설정 
		// String pathname = "D:\choihyeonmin\javastudy"
	    String pathname = "D:/choihyeonmin/javastudy";
